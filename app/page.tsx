import { supabase } from '../lib/supabase'; // 修改为相对路径

export default async function Home({ searchParams }: { searchParams: { q?: string } }) {
  const query = searchParams?.q || '';
  
  // 数据库查询：同时匹配名称和描述中的关键词
        ...
        {tools && tools.length > 0 ? (
          tools.map((tool: any) => (
            <div key={tool.id} className="group bg-white p-8 rounded-[2rem] border border-gray-50 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-500">
              <div className="w-14 h-14 bg-blue-50 text-blue-600 rounded-2xl mb-6 flex items-center justify-center text-xl font-black">
                {tool.name.charAt(0)}
              </div>
