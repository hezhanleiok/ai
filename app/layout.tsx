'use client'; // 必须放在第一行，因为需要处理点击事件

import './globals.css'
import Link from 'next/link'
import { Github, Youtube, Twitter, LogIn, UserPlus } from 'lucide-react'
import { signInWithGithub } from '../lib/auth-client' // 引入刚才写的函数
        ...
        <nav className="sticky top-0 z-50 border-b bg-white/80 backdrop-blur-md">
          <div className="max-w-7xl mx-auto px-4 h-20 flex justify-between items-center">
            ...
            <div className="flex items-center space-x-3">
              <button onClick={() => signInWithGithub()} className="text-sm font-bold text-gray-600 hover:text-blue-600 px-4 py-2">
                登录
              </button>
              <button 
          ...
          <div className="max-w-7xl mx-auto px-4 text-center">
            <div className="flex justify-center space-x-12 mb-10">
              <a href="https://github.com/hezhanleiok/ai" target="_blank" 
                 className="text-gray-400 hover:text-[#24292e] transition-all transform hover:scale-125">
                <Github size={40} />
              </a>
              <a href="https://www.youtube.com/@xiaoheai" target="_blank" 
                 className="text-gray-400 hover:text-[#FF0000] transition-all transform hover:scale-125">
                <Youtube size={40} />
              </a>
              <a href="https://x.com/xiaoheyiyi" target="_blank" 
                 className="text-gray-400 hover:text-[#1DA1F2] transition-all transform hover:scale-125">
                <Twitter size={40} />
              </a>
            </div>
